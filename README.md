# Расчёты диссертации: воспроизводимый проект для VS Code

В этой папке собран Python-код, которым рассчитаны три равноправных примера главы 7 диссертации. Примеры относятся к трём разным системам:

- головной мозг, стерео-ЭЭГ, § 7.2;
- авиационный двигатель, NASA C-MAPSS, § 7.3;
- фондовый рынок, S&P 500, § 7.4.

Кроме того, в папке есть сверка чисел блока E § 3.11 (обучение ГНС на графе объекта) с результатами репозитория автора. Проект открывается в VS Code через файл рабочей области `dissertation-calculations.code-workspace`: окружение, данные, запуск и сверка с результатами диссертации выполняются задачами и конфигурациями запуска.
...
| `src/make_generality_figures.py` | Новые сводные рисунки, не из текста диссертации (раздел 4) → `results/figures/generality/` |
...
Расчётные алгоритмы `src/` не изменены по сравнению с `disser-text/cases/code/`, которыми получены результаты версий 29–30 диссертации. Единственное интерфейсное дополнение — `brain_seeg_pipeline.py` теперь явно распознаёт синтетический демонстрационный набор и помечает его результаты. Текущие контрольные суммы:
...
| `aviation_cmapss_pipeline.py` | `bc194dce9c32c84312b3b82aa8d7cb90` |
| `finance_sp500_pipeline.py` | `0e96a15109be7e46cd9ac3bd97461c09` |
| `make_figures.py` | `e5e5f45b04ae1c495b2403ca4fc25e1a` |
...
| Шаг | Время на 2 ядрах CPU | Где результат | Где в диссертации |
|---|---|---|---|
...
`results/figures/ch7_*.png` (и их состав, Рис. 57–62) не менялись и остаются приложением: по ним видно устройство каждого набора данных так, как это дано в тексте диссертации. Дополнительно `results/figures/generality/` содержит пять новых рисунков — не из текста диссертации, а демонстрацию того, что один и тот же конвейер анализа применим ко всем трём примерам сразу:
...
`scripts/check_block_e.py` читает сохранённые JSON и граф объекта и сверяет с текстом диссертации:
...
- **Сборка md → docx диссертации.** Это не расчёт, скрипты лежат в `disser-text/pipeline/docx_build/`.
