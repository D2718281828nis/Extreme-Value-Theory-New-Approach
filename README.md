...
**Файлы `src/` не изменены** по сравнению с `disser-text/cases/code/`, которыми получены результаты версий 29–30 диссертации. Контрольные суммы:
...
Новый код — только обвязка в `scripts/`. Она задаёт пути, скачивает данные и сверяет результаты, но расчётов не меняет.
